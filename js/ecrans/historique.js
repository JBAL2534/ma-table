...
(function (racine) {
  ...
  function monterSemaine(conteneur, lundi) {
    ...
    const achats = H.achatsActifs(etat).filter(a => U.lundiDe(a.date) === lundi).sort((a, b) => a.date.localeCompare(b.date));
    conteneur.append(el('div', 'carte', el('h3', {}, '🧺 Courses de la semaine'), !achats.length ? el('p', 'sous', 'Aucun passage en magasin terminé cette semaine-là.') :
      el('ul', 'liste', achats.map(a => el('li', {}, el('span', 'ico', C.emojiMagasin(a.magasin)), el('button', { class: 'pousse', style: 'text-align:left;background:none;border:0;padding:0;min-height:44px', onclick: () => feuille({ titre: C.nomMagasin(a.magasin) + ' · ' + U.dateFr(a.date),
          contenu: el('ul', 'liste', a.articles.map(x => el('li', {}, el('span', 'pousse', x.nom, (x.qte != null || x.prixKg) ? el('div', 'minuscule', [x.qte != null ? U.formaterQte(x.qte, x.unite) : null, x.prixKg ? U.euros(x.prixKg) + '/kg' : null].filter(Boolean).join(' · ')) : null), el('span', 'petit', x.prix != null ? U.euros(x.prix) : U.formaterQte(x.qte, x.unite))))),
          actions: [{ libelle: 'Retirer ce passage', classe: 'danger', action: async () => { if (!(await confirmer('Ce passage (' + U.pluriel(a.articles.length, 'article') + (a.montant != null ? ', ' + U.euros(a.montant) : '') + ') ne comptera plus dans l’historique ni dans les prix. Utile après un ticket scanné deux fois.', { ok: 'Retirer', danger: true }))) return false; a.retire = true; a.modifieLe = new Date().toISOString(); sauver(); }, garder: false }, { libelle: 'Fermer', classe: 'principal' }] }) }, el('div', 'gras', C.nomMagasin(a.magasin) + ' · ' + U.nomJour(a.date)), el('div', 'petit', U.pluriel(a.articles.length, 'article') + (a.montant != null ? ' · ' + U.euros(a.montant) : ''))),
  ...
  function monterStats(conteneur, params) {
    ...
    if (variations.length) conteneur.append(el('div', 'carte', el('h3', {}, 'Prix qui ont bougé'), el('p', 'petit', 'D\u2019après vos tickets photographiés.'), el('ul', 'liste', variations.map(v => el('li', {}, el('span', 'ico', v.ecart > 0 ? '📈' : '📉'), el('span', 'pousse', v.nom, el('div', 'petit', U.euros(v.avant) + (v.auKilo ? '/kg' : '') + ' → ' + U.euros(v.apres) + (v.auKilo ? '/kg' : '') + ' · ' + C.nomMagasin(v.magasin))), el('span', { class: 'gras', style: v.ecart > 0 ? 'color:var(--terracotta-fonce)' : 'color:var(--sauge-fonce)' }, (v.ecart > 0 ? '+' : '') + U.euros(v.ecart) + (v.auKilo ? '/kg' : '')))))));
